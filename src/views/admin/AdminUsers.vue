<template>
  <div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          <thead class="bg-gray-800/50 border-b border-gray-800">
            <tr>
              <th class="px-6 py-3 text-gray-300 text-sm">Username</th>
              <th class="px-6 py-3 text-gray-300 text-sm">Role</th>
              <th class="px-6 py-3 text-gray-300 text-sm">Created At</th>
              <th class="px-6 py-3 text-gray-300 text-sm text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="user in users" :key="user._id" class="border-b border-gray-800/50 hover:bg-gray-800/30">
              <td class="px-6 py-3 text-gray-300 text-sm">{{ user.username }}</td>
              <td class="px-6 py-3">
                <span
              ...
              <td class="px-6 py-3 text-gray-400 text-sm">{{ formatDate(user.createdAt) }}</td>
    ...
    <div v-if="modalOpen" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div class="bg-gray-900 rounded-xl max-w-md w-full p-6 border border-gray-800">
        <h3 class="text-xl font-bold text-white mb-4">{{ editingUser ? 'Edit User' : 'New User' }}</h3>
        <form @submit.prevent="saveUser">
          <div class="mb-3">
            <label class="block text-gray-300 text-sm mb-1">Username</label>
            <input v-model="form.username" type="text" required class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white">
          </div>
          <div class="mb-3">
            ...
            <div class="relative">
              <input
                ...
                class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white pr-10"
          ...
          <div class="mb-4">
            ...
            <select v-model="form.role" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white">
          ...
          <div class="flex gap-3">
            <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded">{{ editingUser ? 'Update' : 'Create' }}</button>
...
const openModal = (user?: User) => {
  showPassword.value = false; // reset eye toggle when modal opens
  if (user) {
